...
def delete_product():
    ...
    if not database_name or not product_id:
        return jsonify({
            'error': '缺少必要参数：database_name 或 id'
        }), 400
    ...
    try:
        ...
        with engine.connect() as connection:
            result = connection.execute(delete_statement, {'id': product_id})
            connection.commit()
            # 检查是否有行被删除
            if result.rowcount == 0:
                return jsonify({
                    'message': '未找到对应的产品，删除失败'
                }), 404
